...
class FranceTravailAPI:
    ...
    def get_referential(self, ref_type: str) -> List[Referentiel]:
        ...
        """
        valid_refs = [
            "appellations", "nafs", "communes", "continents", "departements", "domaines",
            "langues", "metiers", "naturesContrats", "niveauxFormations", "pays", "permis",
            "regions", "secteursActivites", "themes", "typesContrats"
        ]
        
        if ref_type not in valid_refs:
            raise ValueError(f"Type de référentiel invalide. Valeurs possibles: {', '.join(valid_refs)}")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for offre in results.resultats:
            ...
            print(f"Lieu: {offre.lieuTravail.libelle if offre.lieuTravail else 'Non précisé'}")
            print(f"Contrat: {offre.typeContratLibelle or 'Non précisé'}")
            print(f"Entreprise: {offre.entreprise.nom if offre.entreprise else 'Non précisé'}")
            print("---")
        ...
        if results.resultats:
            ...
            if offer_details:
                ...
                print(f"Salaire: {offer_details.salaire.libelle if offer_details.salaire else 'Non précisé'}")
