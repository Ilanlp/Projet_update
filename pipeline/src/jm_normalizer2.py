...
class JobDataNormalizer:
    ...
    async def fetch_france_travail_jobs(
        ...
        today_str, yesterday_str = self.get_today_and_yesterday_iso()
        ...
        try:
            while more_results and len(all_jobs) < max_results_to_fetch:
                ...
                search_params = FranceSearchParams(
                    codeROME=code_rome,
                    minCreationDate=yesterday_str,
                    maxCreationDate=today_str,
    ...
    def normalize_adzuna_job(
        ...
        normalized_job = NormalizedJobOffer(
            ...
            code_rome=code_rome,  # Adzuna ne fournit pas cette information
    ...
    async def fetch_and_normalize_adzuna(
        ...
        """
        # S'assurer que les appellations sont chargées (ne devrait être fait qu'une fois)
        if not hasattr(self, "appellations") or self.appellations is None:
        ...
        for code in code_rome.split(","):
            ...
            for search in search_terms:
                search_params.append((search, location, adjusted_max, code))
        ...
        async def rate_limited_api_call(search, location, max_results, code):
            async with api_semaphore:
                # Effectuer l'appel API
                result = await self.fetch_adzuna_jobs(
                    search, location, max_results, code
                )
        ...
        # Traiter les résultats et normaliser les offres d'emploi
        normalized_jobs = []
        for i, raw_jobs in enumerate(raw_results):
            _, _, _, code = search_params[i]
            # Utiliser extend au lieu de += pour une meilleure performance
            normalized_jobs.extend(
                [self.normalize_adzuna_job(job, code) for job in raw_jobs]
            )
    ...
    async def fetch_and_normalize_france_travail(
        ...
        Returns:
            Liste des offres d'emploi normalisées
        """
        # Utiliser la nouvelle méthode de récupération avec pagination
        raw_jobs = await self.fetch_france_travail_jobs(
            code_rome, location, max_results
        )

        logger.info(
            f"Normalisation des {len(raw_jobs)} offres France Travail récupérées"
        )

        # Normaliser chaque offre
    ...
    async def fetch_and_normalize_all(
        ...
        adzuna_jobs = await self.fetch_and_normalize_adzuna(
            code_rome, location_adzuna, max_results
        )

        # Récupérer les données de France Travail
        france_travail_jobs = await self.fetch_and_normalize_france_travail(
            code_rome, location_france_travail, max_results
        )
    ...
    def save_to_csv(
        self, df: pd.DataFrame, filename: str, chunk_size: int = 10000
    ) -> str:
        """
        Sauvegarde le DataFrame dans un fichier CSV, avec support pour les grands volumes de données
        ...
        try:
            # Vérifier si le DataFrame est trop grand pour une sauvegarde directe
            if len(df) > chunk_size:
                logger.info(
                    f"Sauvegarde d'un grand DataFrame ({len(df)} lignes) en mode chunk"
                )

                import csv

                # Utiliser le mode chunksize pour économiser la mémoire
                ...
                # Puis écrire les données par chunks en mode append
                for chunk_start in range(0, len(df), chunk_size):
                    chunk_end = min(chunk_start + chunk_size, len(df))
                    logger.info(f"Écriture du chunk {chunk_start}-{chunk_end}")
    ...
    def get_today_and_yesterday_iso(self):
...
def rechercher_par_mot_cle(df, mot_cle):
    ...
    mask = df["description_adzuna"].str.contains(mot_cle, case=False) | df[
        "description_rome"
    ].str.contains(mot_cle, case=False)
    return df[mask]
...
def obtenir_parametres_recherche(mot_cle):
    ...
    meilleur_code_rome = resultats[
        resultats["categorie_adzuna"] == meilleure_categorie
    ]["code_rome"].iloc[0]
    ...
    print(
        f'python jm_normalizer.py --search "{mot_cle}" --category-adzuna "{meilleure_categorie}" --code-rome "{meilleur_code_rome}"'
    )
...
async def main():
    ...
    location_france_travail = environ.get(
        "DEFAULT_LOCATION_FRANCE_TRAVAIL"
    )  # Code département Paris pour France Travail
    max_results_per_source = 3000

    DEFAULT_CODE_ROME = environ.get("DEFAULT_CODE_ROME_FRANCE_TRAVAIL")
    ...
    async with JobDataNormalizer(
        ...
        # Assurons-nous que le DataFrame est bien indexé pour la performance avec de grands datasets
