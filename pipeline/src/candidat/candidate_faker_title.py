...
class CustomListProvider(BaseProvider):
    ...
    def __init__(self, generator):
        ...
        self.skills = df_dim_competences["skill"].tolist()
        self.softSkills = df_dim_solf_skill["summary"].tolist()
        ...
        for code in self.code_rome_it:
            metiers_it = df_raw_metiers[df_raw_metiers["code_rome"] == code][
                "libelle_search"
            ]
            for m in metiers_it:
                id_metier = df_dim_metiers[df_dim_metiers["ID_APPELLATION"] == m]["NOM"]
                for i in id_metier:
                    self.metiers.append(i)

        # Récupération des données depuis les DataFrames chargés
        self.contracts = df_dim_contrats["type_contrat"].tolist()
        self.domainEntreprises = df_dim_domaines["nom_domaine"].tolist()

        # Définition de listes personnalisées supplémentaires
        self.typeEntreprises = df_dim_type_entreprises["type_entreprise"].tolist()
        self.seniorites = df_dim_seniorites["type_seniorite"].tolist()
        self.teletravails = df_dim_teletravails["type_teletravail"].tolist()
    ...
    def metier(self):
        ...
        max_count = min(3, len(self.metiers))  # Don't request more than available
        if max_count == 0:
            return []  # Return empty list if no métiers available
...
print("Base de données fictive créée et enregistrée dans 'RAW_CANDIDAT.csv'.")
