...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("../../data/adzuna_collector.log"),
...
class AdzunaDataCollector:
    ...
    async def _search_jobs_direct(self, page: int, **params):
        ...
        query_params = {"app_id": self.app_id, "app_key": self.app_key, **params}
        ...
        try:
            ...
            if file_exists:
                ...
                try:
                    ...
                    logger.warning(
                        f"Erreur lors de la lecture du fichier existant: {e}"
                    )
                    has_content = False
            ...
            with open(self.output_file, mode, newline="", encoding="utf-8") as csv_file:
                ...
                if mode == "w":
                    ...
                    logger.info(
                        f"Ajout des données au fichier existant {self.output_file}"
                    )
                        ...
                        while has_more_results:
                            try:
                                ...
                                try:
                                    ...
                                    logger.warning(
                                        f"Erreur API pour la page {page}: {e}"
                                    )
                                ...
                                normalizer = JobDataNormalizer(
                                    ...
                                    os.getenv("FRANCE_TRAVAIL_ID"),
                                    os.getenv("FRANCE_TRAVAIL_KEY"),
                                )

                                # Transforme chaque offre au format CSV et l'écrit
                                for job in filtered_jobs:
                                    # job_data = self._job_to_dict(job)
                                    job_data = normalizer.normalize_adzuna_job(
                                        job.model_dump()
                                    )
            ...
            logger.info(
                f"Collecte terminée! {self.total_collected} offres d'emploi collectées au total."
            )
...
async def main():
    ...
    parser.add_argument(
        "--country", type=str, default="fr", help="Code pays (ex: fr, gb, us)"
    )

    parser.add_argument(
        "--output",
        type=str,
        default=f"{output_path}adzuna_jobs.csv",
        help="Fichier CSV de sortie",
    )

    parser.add_argument(
        "--months", type=int, default=12, help="Nombre de mois à remonter dans le temps"
    )
    ...
    if not app_id or not app_key:
        logger.error(
            "Les identifiants ADZUNA_APP_ID et ADZUNA_APP_KEY doivent être définis dans le fichier .env"
        )
        return 1
    ...
    logger.info(
        f"Démarrage de la collecte des données pour {args.country.upper()} "
