...
def create_matching_pipeline(df):
    ...
    for model_name in param_grid['model_name']:
        for batch_size in param_grid['batch_size']:
            for n_neighbors in param_grid['n_neighbors']:
                # Démarrer un nouveau run pour chaque configuration
    ...
    # Trier et sélectionner le meilleur résultat
    if all_results:
        all_results.sort(key=lambda x: x['avg_score'], reverse=True)
        best_config = all_results[0]
    
    # Démarrer un run final pour le meilleur modèle
    with mlflow.start_run():
        if best_config:
            ...
            mlflow.log_metric('best_score', best_config['avg_score'])
