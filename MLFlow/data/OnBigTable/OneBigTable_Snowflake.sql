...
SELECT
    ...
    LISTAGG(DISTINCT comp.skill, ', ') WITHIN GROUP (ORDER BY comp.skill) AS competences,
    LISTAGG(DISTINCT comp.type, ', ')  WITHIN GROUP (ORDER BY comp.type)  AS types_competences,
...
GROUP BY
    fo.id_local, fo.title, fo.description,

    dc.type_contrat,
    de.nom_entreprise, de.categorie_entreprise, de.date_creation_entreprise,
    dd.code_domaine, dd.nom_domaine,
    dl.code_postal, dl.ville, dl.departement, dl.region, dl.pays, dl.latitude, dl.longitude, dl.population,
    d1.mois, d1.jour, d1.mois_nom, d1.jour_semaine, d1.week_end,
