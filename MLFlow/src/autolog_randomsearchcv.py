...
def main():
    ...
    for run in runs:
        if (
            "best_n_estimators" in run.data.params
        ):  # Parent run has the best_ parameters
            parent_run = run
            break
    ...
    if parent_run:
        ...
        for run in runs:
            if (
                "n_estimators" in run.data.params
                and run.data.params["n_estimators"]
                == best_params_from_parent["n_estimators"]
                and run.data.params["max_depth"] == best_params_from_parent["max_depth"]
                and run.data.params["min_samples_split"]
                == best_params_from_parent["min_samples_split"]
                and run.data.params["min_samples_leaf"]
    ...
    best_run_name = (
        best_run.data.tags.get("mlflow.runName", "Not found")
        if best_run
        else "Not found"
    )
