...
logger.info(f"Script initialisé avec les chemins suivants:")
...
class JobMarketIngestion:
    ...
    def run_normalizer(self):
        ...
        try:
            ...
            if os.getenv('DEFAULT_CODE_ROME_FRANCE_TRAVAIL'):
                cmd.extend(["--code-rome", os.getenv('DEFAULT_CODE_ROME_FRANCE_TRAVAIL')])
            ...
            logger.info(f"Sortie du normalisateur: {result.stdout[:500]}...")  # Afficher les 500 premiers caractères
            
            # Vérifier si des fichiers CSV ont été générés
            files = [f for f in os.listdir(self.output_dir) if f.startswith('all_jobs_') and f.endswith('.csv')]
            if files:
                latest_file = max(files, key=lambda x: os.path.getmtime(os.path.join(self.output_dir, x)))
                latest_csv = os.path.join(self.output_dir, latest_file)
                logger.info(f"Fichier CSV généré: {latest_csv}")
            else:
                logger.warning("Aucun fichier CSV généré par le normalisateur n'a été trouvé")
    ...
    def load_to_snowflake(self):
        ...
        try:
            ...
            logger.info(f"Sortie du chargeur: {result.stdout[:500]}...")  # Afficher les 500 premiers caractères
    ...
    def run_ingestion(self):
        ...
        try:
            ...
            logger.info(f"Processus d'ingestion terminé avec succès en {duration:.2f} secondes (ID: {self.run_id})")
            return True
            
        except Exception as e:
            logger.error(f"Erreur critique lors de l'exécution du processus d'ingestion: {str(e)}")
            import traceback
            logger.error(traceback.format_exc())
            return False
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(sys.argv) > 1:
            if sys.argv[1] == "--test":
                logger.info("Mode test: vérification des chemins et des dépendances sans exécution")
